...
## 1. Статьи: осталось 36 (9 рубрик × 2 темы × RU/EN) 🟢

Партиями по рубрике: пишу RU+EN → `npm run build` → коммит. Стиль — как в готовых:
первый абзац = ответ, `KeyPoints`/`Callout`/`DataTable`, 2–3 `ReadAlso`, `sources`,
`faq`, `translationKey` для пары локалей. Без `cover` — рисуется фирменная заливка.

План тем (слаги RU / EN, общий `translationKey`):
...
## 2. Индексация 🔵 (RU-контент почти вне поиска)

- **Яндекс.Вебмастер — подтвердить права.** Ресурс `https://24zdorovie.com`
  (аккаунт `idartstyle@yandex.ru`): разделы «Индексирование»/«Sitemap» отдавали
  «Недостаточно прав», sitemap не сдан. Файл `public/yandex_6d4d18c8520279dc.html`
  на бою есть (200), но, возможно, от другого аккаунта — посмотреть, какое имя
  файла ждут сейчас, и положить нужный в `public/`. Для RU это главный хвост.
- **Google Search Console** — сдать `sitemap.xml`, ключевые страницы → Request Indexing.
- **Bing Webmaster** — Import from Google Search Console (1 клик).
- **GSC-отчёт** — выгрузить CSV (Запросы/Страницы), агент разберёт (ссылки в панели
  под авторизацией, агент их не видит).

## 3. Монетизация 🔵→🟢

- Регистрации в партнёрках (приоритеты — в `docs/MONETIZATION.md` и в переписке):
  сначала Admitad + маркетплейсы + Фитомаркет Эвалар, затем Здравсити, СберЗдоровье;
  для EN — Amazon Associates и программа добавок.
- После первой ссылки + `erid` — завести оффер в `OFFERS` (`src/config/offers.ts`),
  собрать, задеплоить (агент).
- `<ProductCard />` в статью про давление как образец связки «статья → товар» (агент).
